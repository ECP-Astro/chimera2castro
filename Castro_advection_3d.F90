...
! :::
  ...
  subroutine consup(uin, uin_lo, uin_hi, &
    ...
    use prob_params_module, only : domlo_level, domhi_level, center, dx_level, problo
    ...
    ! Freeze hydro (i.e. zero fluxes) for box defined by coarsest grid at center
    inner_lo = NINT( (center-dx_level(:,0)-problo) / dx_level(:,amr_level) )
    inner_hi = NINT( (center+dx_level(:,0)-problo) / dx_level(:,amr_level) )

    do n = 1, NVAR
       do k = lo(3), hi(3)
          if ( k .ge. inner_hi(3) .and. k .lt. inner_hi(3) ) then
             do j = lo(2), hi(2)
                if ( j .ge. inner_lo(2) .and. j .lt. inner_hi(2) ) then
                   do i = lo(1), hi(1) + 1
                      if ( i .ge. inner_lo(1) .and. i .le. inner_hi(1) + 1 ) then
                         if ( i .eq. inner_lo(1) ) then
                            flux1(i, j, k, n) = MAX( ZERO, flux1(i, j, k, n) )
                         endif
                         if ( i .ge. inner_lo(1) + 1 .and. i .le. inner_hi(1) ) then
                            flux1(i, j, k, n) = ZERO
                         endif
                         if ( i .eq. inner_hi(1) + 1 ) then
                            flux1(i, j, k, n) = MIN( ZERO, flux1(i, j, k, n) )
                         endif
                      endif
                   enddo
                endif
       ...
       do k = lo(3), hi(3)
          if ( k .ge. inner_hi(3) .and. k .lt. inner_hi(3) ) then
             do j = lo(2), hi(2) + 1
                if ( j .ge. inner_lo(2) .and. j .le. inner_hi(2) + 1 ) then
                   do i = lo(1), hi(1)
                      if ( i .ge. inner_lo(1) .and. i .lt. inner_hi(1) ) then
                         if ( j .eq. inner_lo(2) ) then
                            flux2(i, j, k, n) = MAX( ZERO, flux2(i, j, k, n) )
                         endif
                         if ( j .ge. inner_lo(2) + 1 .and. j .le. inner_hi(2) ) then
                            flux2(i, j, k, n) = ZERO
                         endif
                         if ( j .eq. inner_hi(2) + 1 ) then
                            flux2(i, j, k, n) = MIN( ZERO, flux2(i, j, k, n) )
                         endif
                      endif
                   enddo
                endif
       ...
       do k = lo(3), hi(3) + 1
          if ( k .ge. inner_lo(3) .and. k .le. inner_hi(3) + 1 ) then
             do j = lo(2), hi(2)
                if ( j .ge. inner_lo(2) .and. j .lt. inner_hi(2) ) then
                   do i = lo(1), hi(1)
                      if ( i .ge. inner_lo(1) .and. i .lt. inner_hi(1) ) then
                         if ( k .eq. inner_lo(3) ) then
                            flux3(i, j, k, n) = MAX( ZERO, flux3(i, j, k, n) )
                         endif
                         if ( k .ge. inner_lo(3) + 1 .and. k .le. inner_hi(3) ) then
                            flux3(i, j, k, n) = ZERO
                         endif
                         if ( k .eq. inner_hi(3) + 1 ) then
                            flux3(i, j, k, n) = MIN( ZERO, flux3(i, j, k, n) )
                         endif
                      endif
                   enddo
                endif
